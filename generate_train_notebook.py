...
for audio_path in tqdm(test_files, desc="Inference"):
    ...
    # 2. VRAM-Safe Sub-Batched Inference
    with torch.no_grad():
        ...
        for i in range(0, len(images), sub_batch_size):
            ...
            ac_out_list.append(ac_out_sub)
            geo_out_list.append(geo_out_sub)
            
        ac_out = torch.cat(ac_out_list, dim=0)
        geo_out = torch.cat(geo_out_list, dim=0)
